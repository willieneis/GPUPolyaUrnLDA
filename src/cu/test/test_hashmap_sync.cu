...
#ifdef GPULDA_HASH_DEBUG
__device__ inline void debug_print_slot(gpulda::HashMap* m, i32 slot) {
  printf("\nhl:s\tk\tv\tis:st:d\n");
  for(i32 s = slot; s < slot + warpSize/2; ++s) {
    u64 entry = m->data[s % m->capacity];
    printf("%d:%d\t%u\t%d\t", s % 16, s % m->capacity, m->key(entry), m->value(entry));
    if(entry != m->empty()) printf("%d:%d:%d", m->hash_slot(m->key(entry)), m->hash_stride(m->key(entry)), m->key_distance(m->key(entry), slot));
    printf("\n");
  }
  printf("\n");
}
...
__global__ void test_hashmap_init(i32 initial_capacity, i32* map_returned_size) {
  // allocate map
...
__global__ void test_hashmap_insert_print_steps() {
  #ifdef GPULDA_HASH_DEBUG
  // initialize
  __shared__ curandStatePhilox4_32_10_t rng;
  if(threadIdx.x == 0) {
  ...
  if(threadIdx.x < warpSize) {
    if(threadIdx.x == 0) { debug_print_slot(&m, 0); }
    if(threadIdx.x == 0) { debug_print_slot(&m, 16); }
    if(threadIdx.x == 0) { debug_print_slot(&m, 32); }
    if(threadIdx.x == 0) { debug_print_slot(&m, 48); }
    if(threadIdx.x == 0) { debug_print_slot(&m, 64); }
    if(threadIdx.x == 0) { debug_print_slot(&m, 80); }
    if(threadIdx.x == 0) { printf("------------------------------------------------------------\n"); }

    // 16
    m.insert2(threadIdx.x < 16 ? 0 : 3, 1); if(threadIdx.x == 0) { debug_print_slot(&m, 16); printf("------------------------------------------------------------\n"); }
    ...
    // 16->32
    m.insert2(threadIdx.x < 16 ? 48 : 51, 1); if(threadIdx.x == 0) { debug_print_slot(&m, 32); printf("------------------------------------------------------------\n"); }
    ...
    // 48
    m.insert2(threadIdx.x < 16 ? 1 : 4, 1); if(threadIdx.x == 0) { debug_print_slot(&m, 48); printf("------------------------------------------------------------\n"); }
    ...
    // 16->48 evict
    m.insert2(threadIdx.x < 16 ? 96 : 99, 1); if(threadIdx.x == 0) { debug_print_slot(&m, 48); printf("------------------------------------------------------------\n"); }
...
__global__ void test_hashmap_insert2(i32 num_unique_elements, i32 num_elements, i32 max_size, i32* out, i32 rebuild) {
  __shared__ curandStatePhilox4_32_10_t rng;
  if(threadIdx.x == 0) {
    curand_init((unsigned long long) 0, (unsigned long long) 0, (unsigned long long) 0, &rng);
  }
...
void test_hashmap() {
  ...
  // init<warp>
  test_hashmap_init<<<1,warpSize>>>(max_size, out);
  ...
  // init<block>
  test_hashmap_init<<<1,GPULDA_POLYA_URN_SAMPLE_BLOCKDIM>>>(max_size, out);
  cudaDeviceSynchronize() >> GPULDA_CHECK;
  ...
  // print steps
  #ifdef GPULDA_HASH_DEBUG
  test_hashmap_insert_print_steps<<<1,warpSize>>>();
  cudaDeviceSynchronize() >> GPULDA_CHECK;
  return;
  #endif

  // insert2: warp, no rebuild
  test_hashmap_insert2<<<1,warpSize>>>(num_unique_elements, num_elements, max_size, out, false);
  cudaDeviceSynchronize() >> GPULDA_CHECK;
  ...
  test_hashmap_insert2<<<1,GPULDA_POLYA_URN_SAMPLE_BLOCKDIM>>>(num_unique_elements, num_elements, max_size, out, false);
  cudaDeviceSynchronize() >> GPULDA_CHECK;
  ...
  test_hashmap_insert2<<<1,warpSize>>>(num_unique_elements, num_elements, max_size, out, true);
  cudaDeviceSynchronize() >> GPULDA_CHECK;
  ...
  test_hashmap_insert2<<<1,GPULDA_POLYA_URN_SAMPLE_BLOCKDIM>>>(num_unique_elements, num_elements, max_size, out, true);
  cudaDeviceSynchronize() >> GPULDA_CHECK;
