...
extern "C" void sample_phi() {
  polya_urn_sample<<<args->K,256,0,*Phi_stream>>>(Phi->dense, n->dense, args->beta, args->V, alias->prob, alias->alias); // draw Phi ~ PPU(n + beta)
  cudaMemcpyAsync(Phi_temp->dense, Phi->dense, args->V * args->K * sizeof(float), cudaMemcpyDeviceToDevice, *Phi_stream) >> GPLDA_CHECK; // copy Phi for transpose
  cublasSetStream(*cublas_handle, *Phi_stream) >> GPLDA_CHECK; // set the stream, then transpose Phi
  ...
  polya_urn_colsums<<<args->V,128,0,*Phi_stream>>>(Phi->dense, sigma_a, alias->prob, args->K); // compute sigma_a and alias probabilities
  build_alias<<<args->V,32,2*next_pow2(args->K)*sizeof(int), *Phi_stream>>>(alias->prob, alias->alias, args->K); // build Alias table
  cudaMemsetAsync(n->dense, 0, args->K * args->V, *Phi_stream) >> GPLDA_CHECK; // reset sufficient statistics for n
  cudaStreamSynchronize(*Phi_stream) >> GPLDA_CHECK; // don't return until operations completed
}
