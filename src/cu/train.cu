...
extern "C" void initialize(Args* init_args, Buffer* buffers, u32 n_buffers) {
  ...
  for(i32 i = 0; i < n_buffers; ++i) {
    ...
    cudaMalloc(&buffers[i].gpu_d_len, args->max_K_d * sizeof(u32)) >> GPULDA_CHECK;
    cudaMalloc(&buffers[i].gpu_d_idx, args->max_K_d * sizeof(u32)) >> GPULDA_CHECK;
    cudaMalloc(&buffers[i].gpu_K_d, args->max_K_d * sizeof(u32)) >> GPULDA_CHECK;
    cudaMalloc(&buffers[i].gpu_hash, 2*args->max_K_d * sizeof(u64)) >> GPULDA_CHECK;
    cudaMalloc(&buffers[i].gpu_temp, args->max_K_d * sizeof(u32)) >> GPULDA_CHECK;
...
extern "C" void sample_z_async(Buffer* buffer) {
  ...
  cudaMemcpyAsync(buffer->gpu_z, buffer->z, args->buffer_size, cudaMemcpyHostToDevice,*buffer->stream) >> GPULDA_CHECK; // copy z to GPU
  cudaMemcpyAsync(buffer->gpu_w, buffer->w, args->buffer_size, cudaMemcpyHostToDevice,*buffer->stream) >> GPULDA_CHECK; // copy w to GPU
  cudaMemcpyAsync(buffer->gpu_d_len, buffer->d, buffer->n_docs, cudaMemcpyHostToDevice,*buffer->stream) >> GPULDA_CHECK;
  cudaMemcpyAsync(buffer->gpu_K_d, buffer->K_d, buffer->n_docs, cudaMemcpyHostToDevice,*buffer->stream) >> GPULDA_CHECK;
  ...
  warp_sample_topics<<<1,32,0,*buffer->stream>>>(args->buffer_size, buffer->n_docs, buffer->gpu_z, buffer->gpu_w, buffer->gpu_d_len, buffer->gpu_d_idx, buffer->gpu_K_d, buffer->gpu_hash, buffer->gpu_temp, args->K, args->V, args->max_K_d, Phi->dense, alias->prob, alias->alias, buffer->gpu_rng);
  rng_advance<<<1,1,0,*buffer->stream>>>(2*args->buffer_size,Phi_rng);

  // copy z back to host
  cudaMemcpyAsync(buffer->z, buffer->gpu_z, args->buffer_size, cudaMemcpyDeviceToHost,*buffer->stream) >> GPULDA_CHECK;
}
