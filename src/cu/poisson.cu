...
Poisson::Poisson(int ml, int mv) {
  ...
  alias = new SpAlias(max_lambda, max_value);
  // launch kernel to build the alias tables
  build_poisson_prob<<<max_lambda,96>>>(alias->prob, alias->alias, ARGS->beta, max_value);
  cudaDeviceSynchronize() >> GPLDA_CHECK;
  build_alias<<<max_lambda,32,2*next_pow2(max_value)*sizeof(int)>>>(alias->prob, alias->alias, max_value);
  cudaDeviceSynchronize() >> GPLDA_CHECK;
}
