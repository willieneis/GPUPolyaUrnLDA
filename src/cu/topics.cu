...
__global__ void sample_topics(u32 size, u32 n_docs,
  ...
  u64* block_hash = hash + max_N_d*blockIdx.x;
  f32* block_mPhi = mPhi + max_N_d*blockIdx.x;
  ...
  for(i32 offset = 0; offset < n_docs / gridDim.x + 1; ++offset) {
    i32 i  = blockIdx.x + offset * gridDim.x;
    if(i < n_docs) {
      // initialize
      ...
      for(i32 j = 0; j < block_d_len; ++j) {
        ...
        if(u1 * (block_sigma_a + sigma_b) > block_sigma_a) {
          // sample from m*Phi
          block_z = draw_wary_search(u2, m, block_mPhi, sigma_b);
        } else {
